...
static int GenerateDirAndFileIcons(PT_PageLayout ptPageLayout)
{
	...
	PicZoom(&tOriginIconPixelDatas, &g_tDirClosedIconPixelDatas); /* 将缩放后的数据存储起来 */
...
static void BrowsePageRun(PT_PageParams ptParentPageParams)
{
	...
	while(1)
	{
		...
		if(iIndex == -1) /* 触点不在菜单图标上，则判断在哪一个“目录和文件”上 */
		{
			iIndex = GetInputPositionInPageLayout(&g_tBrowsePageDirAndFileLayout, &tInputEvent); /* 索引最大值为当前页目录图标加名字即两倍值 */
			if(iIndex != -1)
			{
				if(g_iStartIndex + iIndex / 2 < g_iDirContentsNumber) /* g_iDirContentsNumber图标速量 iIndex图标加相应名字数量 */
					iIndex += DIRFILE_ICON_INDEX_BASE; /* 这是目录和文件图标，加上DIRFILE_ICON_INDEX_BASE便于区分 */
				else
					iIndex = -1;
			}
		}
		...
		if(tInputEvent.iPressure == 0)
		{
			...
			if(bIconPressed)
			{
				if(iIndexPressed < DIRFILE_ICON_INDEX_BASE) /* 菜单按钮 */
				{
					if(!(bUsedToSelectDir && (iIndexPressed == 1))) /* “选择”图标按钮：来自浏览页面信号 */
					{
						ReleaseButton(&g_atMenuIconsLayout[iIndexPressed]);
					}
					bIconPressed    = 0;

					if(iIndexPressed == iIndex) /* 按下松开为同一图标 */
					{
						switch(iIndex)
						{
							case 0: /* “向上” */
				...
				{
					if(iIndexPressed != iIndex) /* 松开和按下不是同一个图标，则释放图标 */
					{
					...
					else if(bHaveClickSelectIcon) /* 按下和松开都是同一个按钮, 并且"选择"按钮是按下状态 */
                    {
                    	...
                    	/* 如果是目录，记录这个目录 */
                    	iDirFileContentIndex = g_iStartIndex + (iIndexPressed - DIRFILE_ICON_INDEX_BASE)/2; /* 记录索引 */
                    	if(g_aptDirContents[iDirFileContentIndex]->eFileType == FILETYPE_DIR)
                    	{
							ReleaseButton(&g_atMenuIconsLayout[1]); /* 释放选择按钮 */
                    ...
                    else /* “选择”按钮不被按下，单击目录则进入，bUsedToSelectDir为0时单击文件则显示它 */
                    {
                        DeSelectDirFileIcon(iIndexPressed - DIRFILE_ICON_INDEX_BASE);
                        bIconPressed    = 0;
                        ...
                        else if(bUsedToSelectDir == 0) /* 非文件目录，为文件 */
                        {
