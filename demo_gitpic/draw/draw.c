...
int ShowOnePage(unsigned char *pucTextFileMemCurPos)
{
	...
	while (1)
	{
		...
		if (dwCode == '\n')
		{
			...
			/* 回车换行 */
			tFontBitMap.iCurOriginX = 0;
			tFontBitMap.iCurOriginY = IncLcdY(tFontBitMap.iCurOriginY); /* 换行后x肯定为0，y值加上上个字体大小 */
		...
		while (ptFontOpr)
		{
			...
			iError = ptFontOpr->GetFontBitmap(dwCode, &tFontBitMap); /* 获得点阵 */
...
int ShowNextPage(void)
{
	...
	{
		pucTextFileMemCurPos = g_pucLcdFirstPosAtFile; /* 指向第一页的首字符的首地址 */
	}
	iError = ShowOnePage(pucTextFileMemCurPos); /* 将该页显示 */
