...
static void *NetDbgRecvTreadFunction(void *pVoid)
{
	...
	while(1)
	{
		...
		iRecvLen = recvfrom(SocketServerfd,ucRecvBuf,999,0,(struct sockaddr *)&tSocketClientAddr,&iAddrLen);

		if(iRecvLen > 0)
		{
			ucRecvBuf[iRecvLen] = '\0';
			DBG_PRINTF("netprintf.c get msg: %s\n",ucRecvBuf);
...
static int NetDbgInit(void)
{
	int iRet;
	/* 建立服务器端Socket描述符 */
	SocketServerfd = socket(AF_INET, SOCK_DGRAM, 0);
	if(-1 == SocketServerfd)
	{
		printf("socket error!\n");
		return -1;
	...
	g_tSocketServerAddr.sin_family 		= AF_INET;/* TCP/IP */
	g_tSocketServerAddr.sin_port		= htons(SERVER_PORT);
	g_tSocketServerAddr.sin_addr.s_addr	= INADDR_ANY;/* 可以和任何主机通信 */
	memset(g_tSocketServerAddr.sin_zero, 0, 8); /* 填充 */

	/* 捆绑socket描述符 */
	iRet = bind(SocketServerfd, (const struct sockaddr *)&g_tSocketServerAddr, sizeof(struct sockaddr));
	if(-1 == iRet)
	{
		printf("bind error!\n");
		return -1;
